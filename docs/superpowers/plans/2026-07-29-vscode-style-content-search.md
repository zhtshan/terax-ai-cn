...
- [x] Task 1.1 — 核实 `fs_write_file` 原子写路径
- [x] Task 1.2 — `HARD_MAX_RESULTS` 从 2000 提升到 20000
- [x] Task 1.3 — 抽出公共 `build_matcher` helper
- [x] Task 1.4 — 实现 `fs_search_content` IPC 命令
- [x] Task 1.5 — 实现 `fs_replace_all` IPC 命令
- [x] Task 1.6 — 单元测试：`whole_word` 的 regex / 字面量两条路径
- [x] Task 1.7 — 单元测试：`fs_replace_all` 的 secret-path 拒绝路径
- [x] Task 1.8 — 单元测试：`fs_replace_all` 的部分失败返回结构
- [x] Task 1.9 — Rust 全套检查
- [x] Task 2.1 — 在 `src-tauri/src/lib.rs` 注册两个新命令
- [x] Task 3.1 — `src/modules/search/lib/types.ts`：与后端对齐的 TS 类型
- [x] Task 3.2 — `src/modules/search/lib/api.ts`：invoke 包装
- [x] Task 3.3 — `src/modules/search/lib/mode.ts`：选项 → IPC payload
- [x] Task 3.4 — `src/modules/search/lib/highlight.ts`：行内命中切分
- [x] Task 4.1 — `src/modules/search/hooks/useSearchRun.ts`
- [x] Task 4.2 — `src/modules/search/hooks/useReplaceRun.ts`
- [x] Task 4.3 — `src/modules/search/SearchInput.tsx`
- [x] Task 4.4 — `src/modules/search/SearchResults.tsx`
- [x] Task 4.5 — `src/modules/search/ReplaceAffectedBar.tsx`
- [x] Task 4.6 — `src/modules/search/SearchPanel.tsx` 主面板
- [x] Task 4.7 — `src/modules/search/index.ts` 公开导出
- [x] Task 5.1 — `src/modules/sidebar/types.ts` 新增 `"search"` view
- [x] Task 5.2 — `useSidebarPanel.ts` 接受 `"search"` 分支
- [x] Task 5.3 — `SidebarRail.tsx` 新增 Search rail item
- [x] Task 5.4 — `App.tsx` 渲染 `<SearchPanel />` 分支
- [x] Task 5.5 — 切换 tab 不丢失搜索状态
- [x] Task 5.6 — 新增 `Cmd/Ctrl+Shift+F` 快捷键：`search.focusPanel`
- [x] Task 5.7 — 简化 Command Palette `#` 模式：移除 UI 开关
- [x] Task 6.1 — 新增 `searchPanel.*` key（英文）
- [x] Task 6.2 — 新增 `searchPanel.*` key（中文）
- [x] Task 6.3 — 前端静态检查
- [x] Task 6.4 — 手动跑通验收场景 1-15 *(deferred to verify phase; build stage only ensures code/static checks pass)*
- [x] Task 6.5 — Rust 端最终回归
