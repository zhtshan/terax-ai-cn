...
function upstreamBadgeLabel(upstream: string | null | undefined, t: (key: string) => string): string {
  if (!upstream) return t("sourceControl.noUpstreamBadge");
  return upstream;
}
...
export const SourceControlPanel = memo(function SourceControlPanel({
  ...
  const commitDisabledReason = scm.actionBusy
    ...
    : scm.stagedEntries.length === 0
      ? t("sourceControl.stageToEnableCommit")
      : scm.commitMessage.trim().length === 0
        ...
        <header className="flex shrink-0 items-center justify-between gap-2 border-b border-border/50 px-3 pb-2.5 pt-3">
          ...
          <div className="flex shrink-0 items-center gap-0.5">
            ...
            <IconActionButton
              label={
                pullBusy
                  ? t("sourceControl.pulling")
                  : isDiverged
                    ? t("sourceControl.diverged")
                    : !hasUpstream
                      ? t("sourceControl.noUpstream")
                      : (scm.status?.behind ?? 0) === 0
                        ? t("sourceControl.upToDate")
                        : t("sourceControl.pullCommits", { count: scm.status?.behind ?? 0 })
              }
              disabled={!canPull}
              onClick={handlePull}
              side="bottom"
          ...
          <>
            ...
            {scm.allClean ? (
              ...
              <div
                ...
                tabIndex={0}
                role="listbox"
                aria-label={t("sourceControl.changedFiles")}
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {scm.pendingDiscard?.scope === "all"
                ? t("sourceControl.discardAllDesc", { label: scm.pendingDiscard.label })
                : scm.pendingDiscard
                  ? t("sourceControl.discardFileDesc", { label: scm.pendingDiscard.label })
                  : null}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
const EntryRow = memo(function EntryRow({
  ...
  const revealLabel = IS_MAC ? t("sourceControl.revealInFinder") : t("sourceControl.revealInFileManager");
  ...
  return (
    <ContextMenu>
        ...
        >
          ...
          {showDiscard ? (
            <div className="flex shrink-0 items-center opacity-0 transition-opacity group-hover:opacity-100 data-[focused=true]:opacity-100 data-[selected=true]:opacity-100">
              <IconActionButton
                label={t("sourceControl.discardFile", { path: entry.path })}
                disabled={disabled}
                side="top"
          ...
          <span className="flex size-5 shrink-0 items-center justify-center">
            {isStageBusy ? (
              ...
              <Checkbox
                aria-label={t("sourceControl.stageFile", { path: entry.path })}
      ...
      <ContextMenuContent className={COMPACT_CONTENT}>
        ...
        >
          {entry.checkState === "checked" ? t("sourceControl.unstage") : t("sourceControl.stage")}
        </ContextMenuItem>
        {entry.unstaged ? (
          <ContextMenuItem
            className={COMPACT_ITEM}
