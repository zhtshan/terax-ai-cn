...
function relativeTime(ts: number, t: (key: string, opts?: Record<string, unknown>) => string): string {
  ...
  if (s < 60) return t("agents.notification.justNow");
  const m = Math.floor(s / 60);
  if (m < 60) return t("agents.notification.minutesAgo", { n: m });
  const h = Math.floor(m / 60);
  if (h < 24) return t("agents.notification.hoursAgo", { n: h });
  return t("agents.notification.daysAgo", { n: Math.floor(h / 24) });
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {waiting ? t("agents.notification.waiting") : t("agents.notification.working")}
      </span>
    </button>
  );
}
    ...
    <div className="flex items-center gap-2 px-2 py-1">
      ...
      {ready ? (
        ...
        >
          ...
          {installing ? t("agents.notification.enabling") : t("agents.notification.enable")}
        </button>
      )}
    </div>
  );
    ...
    <Popover open={open} onOpenChange={onOpenChange}>
      <PopoverTrigger asChild>
        <Button
          ...
          title={t("agents.notification.bellTitle")}
        >
          <HugeiconsIcon
      ...
      >
        ...
        {empty ? (
          <div className="border-t border-border/60 px-3 py-5 text-center text-xs leading-relaxed text-muted-foreground">
            {t("agents.notification.emptyTitle")}
            <br />
            {t("agents.notification.emptyDesc")}
          </div>
        ) : (
        ...
        <div className="border-t border-border/60 p-1">
          ...
          >
            ...
            {t("agents.notification.alerts")}
            <span className="ml-auto flex items-center gap-1.5 normal-case tracking-normal">
              {enabledCount > 0 ? (
                <span className="text-[10px] text-muted-foreground/60">
                  {t("agents.notification.on", { count: enabledCount })}
                </span>
              ) : null}
              <HugeiconsIcon
