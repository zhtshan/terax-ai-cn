    ...
    <TooltipProvider delayDuration={500} skipDelayDuration={200}>
      <div className="flex h-full min-h-0 flex-col bg-background [contain:layout_style]">
        {loadStatus === "initial" && commits.length === 0 ? (
          ...
          <CenterPlaceholder>
            <div className="text-[13px] font-medium">{t("gitHistory.noCommitsTitle")}</div>
            <div className="max-w-md text-[11px] leading-relaxed text-muted-foreground">
          ...
          <>
            ...
            >
              <div />
              <div className="pl-px">{t("gitHistory.header.sha")}</div>
              <div className="min-w-0">{t("gitHistory.header.subject")}</div>
              <div />
              <div className="ml-2">{t("gitHistory.header.author")}</div>
              <div className="text-right">{t("gitHistory.header.date")}</div>
              <div className="text-right">{t("gitHistory.header.changes")}</div>
            </div>
            <div
              ref={scrollRef}
    ...
    >
      ...
      >
        ...
        <span className="min-w-0 truncate">
          {commit.author ? highlight(commit.author, query) : t("gitHistory.unknown")}
        </span>
      </span>
      ...
      <span className="flex min-w-0 items-center justify-end gap-1.5 font-mono text-[10px] tabular-nums">
        {commit.filesChanged > 0 ? (
          <span
            ...
            title={t("gitHistory.filesChanged", { count: commit.filesChanged })}
          >
            <HugeiconsIcon
              icon={File02Icon}
              size={10.5}
    ...
    <div className="flex max-h-[60vh] min-h-0 flex-col">
      <div className="shrink-0 border-b border-border/45 p-3">
        ...
        <div className="mt-2 flex min-w-0 items-center gap-1.5 text-[10.5px] text-muted-foreground">
          <span className="truncate">{commit.author || t("gitHistory.unknown")}</span>
          {commit.authorEmail ? (
            <>
