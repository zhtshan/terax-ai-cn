...
function Toolbar({ block, all, onSearch }: ChromeProps) {
  ...
  return (
    <div className="bt-tools">
      {failed && <span className="bt-exit">{t("terminal.overlay.exit", { code: block.exitCode })}</span>}
      {duration && <span className="bt-dur">{duration}</span>}
      {!block.running && !!block.command && (
        <button
          type="button"
          title={t("terminal.overlay.runAgain")}
          className="bt-btn"
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button type="button" title={t("terminal.overlay.blockActions")} className="bt-btn">
          <HugeiconsIcon
      ...
      >
        <MenuItem
          icon={Refresh01Icon}
          label={t("terminal.overlay.runAgain")}
          disabled={block.running || !all.promptReady || !block.command}
          onClick={() => all.onRunAgain(block.command)}
        />
        <MenuItem
          icon={Copy01Icon}
          label={t("terminal.overlay.copyCommand")}
          disabled={!block.command}
          onClick={() => copy(block.command, t("terminal.overlay.commandCopied"))}
        />
        <MenuItem
          icon={ComputerTerminal02Icon}
          label={t("terminal.overlay.copyOutput")}
          onClick={() => {
            const o = output();
            if (o) copy(o, t("terminal.overlay.outputCopied"));
          }}
        />
        <MenuItem
          icon={Copy01Icon}
          label={t("terminal.overlay.copyCommandAndOutput")}
          onClick={() => {
            const text = `$ ${block.command}\n${output()}`;
            copy(text, t("terminal.overlay.blockCopied"));
          }}
        />
        <MenuItem
          icon={SparklesIcon}
          label={t("terminal.overlay.attachToAi")}
          onClick={attach}
        />
        <MenuItem
          icon={Search01Icon}
          label={t("terminal.overlay.findInBlock")}
  ...
  return (
    <div className="bt-search pointer-events-auto">
      ...
      <input
        ...
        placeholder={t("terminal.overlay.findInBlock")}
